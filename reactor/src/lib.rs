...
{
    pub fn new<F: (FnOnce(InitContext<Matter>) -> Matter) + Sync + Send + 'static>(init: F) -> Self {
        Self(core::Core::create(init)) 
    }

    pub fn async_new<F>(init: F) -> Self 
    where F: FnOnce(InitContext<Matter>) -> BoxFuture<'static, Matter> + Sync + Send + 'static
    {
    ...
    where D: Sync + Send, F: Fn(Context<Matter>) -> D + Sync + Send + 'static {
...
mod tests {
    ...
    pub async fn test_measure_no_reaction() {
        let reactor = Reactor::<Foo>::new(|ctx| { 
            Foo {
    ...
    pub async fn test_ray() {
        ...
        let mut measure = reactor.use_stabilised_measure(false, |ctx| ctx.ray.to_owned()).await;
        assert!(!measure.to_owned());
