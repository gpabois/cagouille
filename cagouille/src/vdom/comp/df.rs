...
impl AnyComponentStateDf {
    fn downcast<Comp: Component<M> + 'static, M: Mode>(self) -> Option<StateDf<M, Comp>> where Comp::Properties: Differentiable {
        self.0.downcast::<StateDf<M, Comp>>().ok().map(|b| *b)
    }
}

impl<M: Mode> AsyncDifferentiable for AnyComponentNode<M> {
    type Df = AnyComponentDf;

    fn df<'a, 'fut>(src: &'a AnyComponentNode<M>, dest: &'a AnyComponentNode<M>) -> LocalBoxFuture<'fut, AnyComponentDf> where 'a: 'fut {
        Box::pin(async {
