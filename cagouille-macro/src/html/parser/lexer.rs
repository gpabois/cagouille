...
impl<'a> Lexer<'a> {
    ...
    fn consume(&mut self) -> Option<Result<(Class, Value), LexerError>> {
...
impl VNodeToken {
    pub fn next(input: ParseStream) -> Option<Result<(Class, Value), LexerError>> {
        if input.is_empty() {
            return None;
        }

        let res: Result<(Class, Value), LexerError> = (move || {
